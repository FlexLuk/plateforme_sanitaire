...
namespace LYRA
{
    ...
    {
        public static ExcelPackage DownloadAllJournalier(List<RapportParamList> fModels)
        {
        ...
        public static ExcelPackage DownloadFamJournalier(List<FichisteModel> fModels)
        {
            ...
            int lastColumn = 12;
            ExcelRange rg = worksheet.Cells[firstRow, firstColumn, lastRow, lastColumn];
            string tableName = "RAPPORT PARAMÈTRE";
            //Ading a table to a Range
            ExcelTable tab = worksheet.Tables.Add(rg, tableName);
            //Formating the table style
            tab.TableStyle = TableStyles.Light8;
            ...
            foreach (var fModel in fModels)
            {
                ...
                worksheet.Cells[i + 1, 3].Value = fModel.FDateCreate.Value.ToShortTimeString();
                worksheet.Cells[i + 1, 4].Value = fModel.FName;
                worksheet.Cells[i + 1, 5].Value = fModel.FEtablissement;
                worksheet.Cells[i + 1, 6].Value = fModel.FCategorie;
                worksheet.Cells[i + 1, 7].Value = fModel.FFrequenceM;
                worksheet.Cells[i + 1, 8].Value = fModel.FFrequenceY;
                worksheet.Cells[i + 1, 9].Value = fModel.FParametre;
                worksheet.Cells[i + 1, 10].Value = fModel.FNomfichiste;
                i++;
            }
